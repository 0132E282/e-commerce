...
class PaymentMoMo implements PaymentInterface
{
    ...
    {
        $this->secretKey = $config['secretKey'] ?? env('MOMO_ACCESS_KEY');
        $this->endpoint =  $config['endpoint'] ?? env('MOMO_ENDPOINT');
        $this->inputData['accessKey'] = $config['accessKey'] ?? env('MOMO_ACCESS_KEY');
        $this->inputData['amount'] = $req['amount'] ?? '';
        $this->inputData['extraData'] = '';
        $this->inputData['ipnUrl'] =  $config['ipnUrl'] ?? env('MOMO_IPN_URL ');
        $this->inputData['orderId'] = $req['order_id'] ?? '';
        $this->inputData['orderInfo'] = $req['content_order'] ?? '';
        $this->inputData['partnerCode'] =  $config['partnerCode'] ?? env('MOMO_PARTNER_CODE');
